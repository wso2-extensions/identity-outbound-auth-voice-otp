...
<%@ page import="org.wso2.carbon.identity.authenticator.voiceotp.VoiceOTPUtils" %>
...
<%
    ...
    if (Boolean.parseBoolean(request.getParameter(Constants.AUTH_FAILURE))) {
        ...
        if (request.getParameter(Constants.AUTH_FAILURE_MSG) != null) {
            ...
            } else if (errorMessage.equalsIgnoreCase(VoiceOTPConstants.ERROR_CODE_MISMATCH)) {
                errorMessage = IdentityManagementEndpointUtil.i18n(resourceBundle,"error.code");
            } else if (errorMessage.equalsIgnoreCase(VoiceOTPConstants.ERROR_VOICEOTP_DISABLE_MSG)) {
                errorMessage = IdentityManagementEndpointUtil.i18n(resourceBundle,"error.voiceotp.disabled");
            } else if (errorMessage.equalsIgnoreCase(VoiceOTPConstants.TOKEN_EXPIRED_VALUE)) {
                errorMessage = IdentityManagementEndpointUtil.i18n(resourceBundle,"error.token.expired");
            } else if (errorMessage.equalsIgnoreCase(VoiceOTPConstants.SEND_OTP_DIRECTLY_DISABLE_MSG)) {
                errorMessage = IdentityManagementEndpointUtil.i18n(resourceBundle,"error.user.not.found.voiceotp");
            ...
            } else if (VoiceOTPUtils.useInternalErrorCodes()) {
                String httpCode = URLDecoder.decode(errorMessage, VoiceOTPConstants.CHAR_SET_UTF_8);
                errorMessage = VoiceOTPConstants.ErrorMessage.getMappedErrorMessage(httpCode);
            }
        }
        if (request.getParameter(VoiceOTPConstants.AUTH_FAILURE_INFO) != null) {
            errorInfo = IdentityManagementEndpointUtil.i18n(resourceBundle,new
                    String(getDecoder().decode(request.getParameter(VoiceOTPConstants.AUTH_FAILURE_INFO))));
        ...
        <layout:main layoutName="<%= layout %>" layoutFileRelativePath="<%= layoutFileRelativePath %>" data="<%= layoutData %>" >
            ...
            <layout:component componentName="MainSection" >
                <div class="ui segment">
                    <%-- page content --%>
                    <h2><%=IdentityManagementEndpointUtil.i18n(resourceBundle, "error.failed.with.voiceotp")%></h2>
                    <%
